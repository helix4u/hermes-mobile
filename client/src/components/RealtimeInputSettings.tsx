      ...
      try {
        ...
        if (isNativeHermesClient()) {
          try {
            ...
            if (alive) setNativeInventory(inventory.inputs.map(input => `${input.label} (${names[input.type] || 'Audio input'}${input.channels.length ? `, ${input.channels.join('/')} ch` : ''})`))
          } catch { /* Older native hosts retain the working WebView picker. */ }
        }
